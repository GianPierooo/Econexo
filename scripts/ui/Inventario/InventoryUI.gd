...
func _ready():
	...
	ContextMenu.on_inspect.connect(_on_inspect)
	ContextMenu.on_scan.connect(_on_scan)
...
func _on_slot_clicked(fragment_data):
	...
	ContextMenu.show_menu(fragment_data, mouse_pos)

func _on_inspect(fragment_data):
	print("Abriendo inspección: ", fragment_data["nombre"])
	InspectionUi.mostrar(fragment_data)
